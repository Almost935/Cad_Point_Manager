...
namespace Cad_Point_Manager.Models.DrawingObjects3D
{
    public class DrawingMtext3D : DrawingObject3D
    {
        ...
        public List<DrawingMtextSegment3D> GetMtextSegments()
        {
            ...
            //int currentFontSize = (int)Math.Ceiling(DxfMtext.Height * 1.25);
            ...
            foreach (Match match in matches)
            {
                ...
                if (Regex.IsMatch(value, colorPattern))
                {
                    int colorI = int.Parse(Regex.Match(value, colorPattern).Groups[1].Value);
            ...
            foreach (var segment in formattedSegments)
            {
                if (segment.text == "\n")
                {
                    yOffset -= (float)(segment.textHeight * 1.2);
                    xOffset = 0;
                }
                if (!string.IsNullOrWhiteSpace(segment.text))
                {
                    ...
                    Text text = new(segment.text, pos, segment.textHeight);
                    DrawingMtextSegment3D mtextSegment = new(this, segment.text, segment.color, new Vector3(basePosition.X + xOffset, basePosition.Y + yOffset, 0), Rotation, (int)Math.Ceiling(segment.textHeight * 1.25), segment.font, (float)(text.Height), (float)text.Width, segment.italic, segment.bold, segment.underline, segment.strikethrough);

                    mtextSegmentList.Add(mtextSegment);
                    xOffset += (float)text.Width;
