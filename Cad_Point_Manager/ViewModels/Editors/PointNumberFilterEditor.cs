...
{
    public sealed class PointNumberFilterEditor : INotifyPropertyChanged, INotifyDataErrorInfo
    {
        ...
        public bool TryGetRange(out int min, out int max)
        {
            min = 0; max = 0;
        ...
        private void Validate()
        {
            ...
            // Min
            if (string.IsNullOrWhiteSpace(MinText)) { AddError(nameof(MinText), "Min point number is required."); }
            else if (!int.TryParse(MinText, out _)) { AddError(nameof(MinText), "Min must be an integer."); }

            // Max
            if (string.IsNullOrWhiteSpace(MaxText)) { AddError(nameof(MaxText), "Max point number is required."); }
            else if (!int.TryParse(MaxText, out _)) { AddError(nameof(MaxText), "Max must be an integer."); }

            // Range rule (only if both parse)
            if (int.TryParse(MinText, out var min) && int.TryParse(MaxText, out var max))
            {
                if (min > max)
                {
